...
class Gameboard {
  ...
  autoPlaceShip (ship) {
    ...
    for (const coordinate of coordinates) {
      ...
      // Diagonals
      if (y + 1 <= 9 && x - 1 >= 0) if (this.cells[x - 1][y + 1].ship !== null) return this.autoPlaceShip(ship) // SW
      if (y + 1 <= 9 && x + 1 <= 9) if (this.cells[x + 1][y + 1].ship !== null) return this.autoPlaceShip(ship) // SE
      if (y - 1 >= 0 && x - 1 >= 0) if (this.cells[x - 1][y - 1].ship !== null) return this.autoPlaceShip(ship) // NW
      if (y - 1 >= 0 && x + 1 <= 9) if (this.cells[x + 1][y - 1].ship !== null) return this.autoPlaceShip(ship) // NE
    }
